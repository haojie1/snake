...
private:
	...
	SDL_Color _headColor, _bodyColor;
public:
	snake(int gridNumberRow, int gridNumberCol, float head_x = 0, float head_y = 0) :
		...
		_speed(baseSpeed), _alive(true), _headColor({255, 255, 0}), _bodyColor({255, 255, 255}), _dire(DOWN) {
	...
	const SDL_Color& getHeadColor() const {return _headColor;}
	const SDL_Color& getBodyColor() const {return _bodyColor;}
	void setHeadColor(const SDL_Color& color);
	void setBodyColor(const SDL_Color& color);
